...
def _frame_analysis_system_prompt() -> str:
    return (
        ...
        "the required base layers at their anchors. Reference content can be recorded as "
        "additional middle-layer or final-overlay evidence, but this analysis does not decide "
        "whether it appears in the generated final frame. "
        ...
        "locked_text. Use importance=core for the strongest observed performance nodes or a "
        "small causally linked cluster: prioritize distinctive camera escalation or reframing, "
        "subject-action intensity, causal clarity, interaction payoff, effects or lighting peaks, "
        "and a visible consequence. A core node may occur anywhere in the reference timeline; "
        "do not equate the latest timestamp with the strongest node. Mark connective material as "
        "supporting and incidental texture as decorative so the later director can compress them "
        "before weakening a core node. Use numeric seconds only. reference_end_second must be "
        "strictly greater than reference_start_second. Every beat_id must be non-empty and unique. "
        "depends_on "
        ...
        "preserve_through_last_anchor only to record that a reference element remains visible "
        "through the observed reference ending and is a candidate for director review. This "
        "mapping is not a generated-final-frame requirement. Its instruction must describe the "
        "observed persistence and safe placement if later selected by the director. Use "
        "replace_with_target or morph_to_target when a target frame visibly "
...
def _frame_anchored_director_system_prompt() -> str:
    return (
        ...
        "the one or few signature moments with the strongest recognizability and commercial "
        "impact. "
        ...
        "executed with the available target assets. For each observed overlay or UI element, "
        "must_remain_visible_until_final describes the observed reference ending only. "
        "overlay_lifecycle_plan is the sole generated-final-frame authority: decide dynamically "
        "whether to inherit the element, replace it with the target equivalent, persist it to "
...
def _frame_anchored_storyboard_text_system_prompt() -> str:
    return (
        ...
        "rhythm, VFX timing, transitions, UI, text, rewards, and element lifecycle. When "
        "reference_video_analysis exists, actively identify the strongest compatible performance "
        "node or small cluster of causally linked nodes from the observed core beats and "
        "chronological segments. Rank them dynamically by distinctiveness and commercial "
        "attention value, including camera acceleration or reframing, subject-action intensity, "
        "causal clarity, interaction payoff, VFX or lighting peak, and visible UI, text, reward, "
        "or state-change consequence; do not use a fixed action or climax template. Make the "
        "selected strong node the dominant dramatic spine of the middle rather than optional "
        "decoration. Preserve or adapt its recognizable approach, action or impact, and visible "
        "consequence whenever physically feasible and compatible with the endpoint anchors; omit "
        "it only for a concrete conflict, not merely to simplify continuity. Do not flatten an "
        "evidence-backed strong node into a generic slow pull-back, idle pose, restrained gesture, "
        "uniform camera drift, or ambient particles. Allocate enough relative screen time and "
        "intensity for readability using minimum_readable_duration_seconds and dependencies as "
        "guidance, while compressing or merging supporting beats first. Remap reference timing "
        "to duration_seconds dynamically; reference seconds establish order and relative rhythm, "
        "not target timestamps. Endpoint anchors govern opening and ending visual truth, but they "
        "must not suppress compatible middle action, camera, or VFX intensity. Adapt, replace, "
        "preserve, or omit supporting ideas according to the target images, target identity, "
...
def _frame_anchored_storyboard_system_prompt() -> str:
    return (
        ...
        "target plan defines the target scene windows. When director_plan exists, "
        "action_arc_windows and climax_beats are the sole target-timing authority; draft seconds "
        "are creative hints only. Do not mechanically copy reference seconds "
        "or linearly scale them. Preserve the dependency order and allocate readable time to core "
        "causal beats; compress repeated or decorative material first and extend readability or "
        "the "
        ...
        "When director_plan.signature_moment_plan exists, every item whose strategy is "
        "preserve or adapt or replace_with_equivalent must be executed inside its assigned "
        "core beat. Convert "
        "its source_evidence and target_adaptation into actual camera behavior, subject "
        "performance, effect timing, and visible result as applicable; do not merely label "
        "the beat "
        "or summarize the signature moment. Realize the adapted signature inside the one "
        "continuous "
        "shot without editing, stitching, or post-production assembly. Use neither a stage "
        "name nor "
        ...
        "permit it. Use preserve, replace_with_target, morph_to_target, endpoint_only, and "
        "preserve_through_last_anchor only as identity-substitution evidence. Do not treat an "
        "observed must_remain_visible_until_final value or preserve_through_last_anchor mapping "
        "as a target final-frame command. When director_plan exists, overlay_lifecycle_plan is the "
        "sole "
        "generated-final-frame authority. Every non-null scene overlay_instruction must match its "
        "reference_element to that plan and copy the director strategy, timing_instruction, and "
        "final_frame_requirement; otherwise return null. Include optional per-scene sound_effects "
        "plus overall music and ambience "
        "directions "
        ...
        "continuity, (2) director-selected reference lifecycle and causal behavior, including any "
        "director-approved final overlays, (3) the advertising objective where it does not "
        "contradict supplied images, and "
