...
7. 若不足目标数量，后台汇总历史 query、技术淘汰原因、模型排除原因和重复候选数量并交给模型生成下一轮查询；后台对 query 做跨轮去重，达到上限后返回 `insufficient`。
...
后台对技术过滤记录通用原因码，例如 `status_not_active`、`missing_video`、`missing_thumbnail`、`video_url_rejected`、`thumbnail_url_rejected`、`active_days_below_minimum`、`duration_unavailable` 和 `duration_over_30`。最终汇总只返回原因计数和每轮统计，不默认回传全部被技术淘汰广告的完整内容。
...
- 每条最终广告均满足 ACTIVE、有视频、视频不超过 30 秒、投放时间大于等于 1 天、ID 无重复；
